...
import com.Jayce.Redis.Utils.JedisUtil;
...
@ContextConfiguration(locations={"classpath:applicationContext.xml","classpath:applicationContext-ActiveMQ.xml","classpath:applicationContext-Redis.xml"}) //加载配置文件
public class JedisFullTest {

    @Test
    public void Hello() {
        Jedis jedis = JedisUtil.getJedis("127.0.0.1",6379,"admin123");;
        jedis.auth("admin123");
        try {
            ...
            e.printStackTrace();
//        } finally {
//            JedisUtil.getPool("127.0.0.1",6379).returnResource(jedis);
        }
...
//            JedisUtil.getPool("127.0.0.1",6379).returnResource(jedis);
        ...
        // 排序(排序必须使用SortingParams，不然会报错：redis.clients.jedis.exceptions.JedisDataException: ERR One or more scores can't be converted into double)
        SortingParams sortingParameters = new SortingParams();
        sortingParameters.alpha();
        sortingParameters.limit(0, 3);
        System.out.println(jedis.sort("排序结果："+"lists",sortingParameters));
        // 字串
        System.out.println(jedis.lrange("List转化为String："+"lists", 0, 3));
        // 修改列表中单个值
    ...
    public void testSet() {
        ...
        try {
            ...
            // 移除集合中一个或多个成员(此处为删除第四个成员)
            jedis.srem("myset", "4");
            System.out.println("获取所有加入的value:"+jedis.smembers("myset"));// 获取所有加入的value
            System.out.println("判断 minx:"+jedis.sismember("myset", "4"));// 判断 minxr
            // 是否是sname集合的元素
            System.out.println("是否是sname集合的元素:"+jedis.scard("sname"));// 返回集合的元素个数
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
//            JedisUtil.getPool("127.0.0.1",6379).returnResource(jedis);
        ...
        // 移除并返回集合中的一个随机元素
        System.out.println("spop方法："+jedis.spop("sets"));
        System.out.println("smembers方法："+jedis.smembers("sets"));
        //
...
//            JedisUtil.getPool("127.0.0.1",6379).returnResource(jedis);
    ...
    public void testHsh() {
        ...
        try {
            ...
            System.out.println("lrange获取到的values:"+values);
            //获取 set 中的值
            Set<String> setValues = jedis.zrange("hackers", 0, -1);
            setValues = jedis.hkeys("kid");
            System.out.println("zrange:"+setValues);
            values = jedis.hvals("kid");
            System.out.println(values);
            pairs = jedis.hgetAll("kid");
            System.out.println("jedis.hgetAll();获取到的值："+pairs);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
//            JedisUtil.getPool("127.0.0.1",6379).returnResource(jedis);
...
//        JedisUtil.getPool("127.0.0.1",6379).returnResource(jedis);
    ...
    public void testSort1() {
        ...
        Jedis redis = JedisUtil.getJedis("127.0.0.1",6379,"admin123");
        redis.auth("admin123");
        ...
        redis.flushDB();
        JedisUtil.closeJedis(redis,"127.0.0.1",6379);
    }
